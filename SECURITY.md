...
Execution is split into a separate `latch-agent-executor` service. The bridge remains a planning/reporting process; the executor polls approved non-sensitive command approvals with `executionMode` set to `shell` or `browser`, runs the exact stored plan once, and records an execution audit. The executor runs as a dedicated **non-root** user with an isolated writable area and systemd hardening (`NoNewPrivileges`, `PrivateTmp`, `ProtectSystem=full`) so an approved-but-wrong plan has a bounded blast radius. `screenshot`/`download` paths are confined to its download directory.
...
- **`external_contact` (draft-only, or host-brokered send).** For operator-identity messages. The agent drafts in Latch. With `sendMode: "manual"` approval does **not** send — you send by hand. With `sendMode: "approved_connector"` (the "draft a reply" flow, see [DRAFT-REPLIES.md](./DRAFT-REPLIES.md)) the trusted host sends the (operator-edited) reply from *your* address via a send-only connector **after your approval** — the worker never holds the credential or the send button, and this send is always human (never auto-approved or grantable).
